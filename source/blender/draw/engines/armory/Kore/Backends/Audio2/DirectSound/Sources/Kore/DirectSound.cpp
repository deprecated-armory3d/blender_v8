...
void Audio2::init() {
	...
	dsound->SetCooperativeLevel((HWND)System::windowHandle(0), DSSCL_PRIORITY);
	...
	dsound->CreateSoundBuffer(&bufferDesc, &dbuffer, nullptr);

	DWORD size1;
	u8* buffer1;
	//affirm(dbuffer->Lock(writePos, gap, (void**)&buffer1, &size1, nullptr, nullptr, 0));
	dbuffer->Lock(writePos, gap, (void**)&buffer1, &size1, nullptr, nullptr, 0);
	for (int i = 0; i < gap; ++i) buffer1[i] = 0;
	//affirm(dbuffer->Unlock(buffer1, size1, nullptr, 0));
	dbuffer->Unlock(buffer1, size1, nullptr, 0);
...
void Audio2::update() {
	...
	dbuffer->GetCurrentPosition(&playPosition, &writePosition);
	...
	dbuffer->Lock(writePos, gap, (void**)&buffer1, &size1, (void**)&buffer2, &size2, 0);
	...
	dbuffer->Unlock(buffer1, size1, buffer2, size2);
